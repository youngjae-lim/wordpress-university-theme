...
class Like {
  ...
  ourClickDispatcher(e) {
    let currentLikeBox = e.target; // Get the wrapping parent element having 'like-box' class
  ...
  async createLike(currentLikeBox) {
    try {
      ...
      if (response.data != 'Only logged in users can create a like.') {
        currentLikeBox.setAttribute('data-exists', 'yes'); // Parse string to integer

        var likeCount = parseInt(currentLikeBox.querySelector('.like-count').innerHTML, 10); // Increase the number of likes by 1

        likeCount++; // Update the number of likes

        currentLikeBox.querySelector('.like-count').innerHTML = likeCount; // Set 'data-like' attribute to the id of the created like post
  ...
  async deleteLike(currentLikeBox) {
    try {
      ...
      currentLikeBox.setAttribute('data-exists', 'no'); // Parse string to integer

      var likeCount = parseInt(currentLikeBox.querySelector('.like-count').innerHTML, 10); // Decrease the number of likes by 1

      likeCount--; // Update the number of likes

      currentLikeBox.querySelector('.like-count').innerHTML = likeCount; // Reset 'data-like' attribute to empty
...
class MyNotes {
  ...
  clickHandler(e) {
    ...
    if (e.target.classList.contains('delete-note') || e.target.classList.contains('fa-trash-o')) this.deleteNote(e); // Fire editNote() when 'Edit' button is clicked

    if (e.target.classList.contains('edit-note') || e.target.classList.contains('fa-pencil') || e.target.classList.contains('fa-times')) this.editNote(e); // Fire updateNote() when 'Save' button is clicked
  ...
  editNote(e) {
    ...
    const thisNote = this.findNearestParentLi(e.target); // Toggle the edit/cancel button
  ...
  makeNoteEditable(thisNote) {
    ...
    thisNote.querySelector('.edit-note').innerHTML = '<i class="fa fa-times" aria-hidden="true"></i> Cancel'; // Make note title and body editable

    thisNote.querySelector('.note-title-field').removeAttribute('readonly');
    thisNote.querySelector('.note-body-field').removeAttribute('readonly'); // Add note-active-field class to both note title and body

    thisNote.querySelector('.note-title-field').classList.add('note-active-field');
    thisNote.querySelector('.note-body-field').classList.add('note-active-field'); // Make 'save' button visible

    thisNote.querySelector('.update-note').classList.add('update-note--visible'); // Update data-state to 'editable'
  ...
  makeNoteReadOnly(thisNote) {
    ...
    thisNote.querySelector('.edit-note').innerHTML = '<i class="fa fa-pencil" aria-hidden="true"></i> Edit'; // Make the note title and body read only

    thisNote.querySelector('.note-title-field').setAttribute('readonly', 'true');
    thisNote.querySelector('.note-body-field').setAttribute('readonly', 'true'); // Make the note title and body non-active

    thisNote.querySelector('.note-title-field').classList.remove('note-active-field');
    thisNote.querySelector('.note-body-field').classList.remove('note-active-field'); // Hide 'Save' button

    thisNote.querySelector('.update-note').classList.remove('update-note--visible'); // Update data-state to 'cancel'
  ...
  async updateNote(e) {
    const thisNote = this.findNearestParentLi(e.target); // Get the updated title and body content
    ...
    try {
      ...
      const response = await axios__WEBPACK_IMPORTED_MODULE_0___default().post(universityData.root_url + '/wp-json/wp/v2/note/' + thisNote.getAttribute('data-id'), ourUpdatedPost); // As soon as a user click 'Save' button, make the note read only
  ...
  async createNote() {
    ...
    try {
      ...
      if (response.data != 'You have reached your note limit.') {
        ...
        document.querySelector('.new-note-body').value = ''; // Add the newly added note on top of the existing notes
...
class Search {
  ...
  events() {
    ...
    this.closeButton.addEventListener('click', () => this.closeOverlay()); // Add keydown event to document; press s to open up the overlay

    document.onkeydown = e => this.keyPressDispatcher(e); // Add keyup event to the searchField; logic for showing a spinner loader
...
/******/ 		}
/******/ 		
/******/ 		var chunkLoadingGlobal = self["webpackChunkuniversity_theme"] = self["webpackChunkuniversity_theme"] || [];
